document.getElementById('generateHardButton').addEventListener('click', generateRandomPhases);
...
function generateRandomPhases() {
  ...
  for (var i = 1; i <= 10; i++) {
    ...
    var randomPhase2 = getRandomItem(phases);
    var phaseText1 = randomPhase1.replace("{num}", '<span class="phase-number-italic">' + getRandomNumber(3, 6) + '</span>');
    var phaseText2 = randomPhase2.replace("{num}", '<span class="phase-number-italic">' + getRandomNumber(2, 6) + '</span>');

    var phaseTextElement = document.createElement('span');
    phaseTextElement.classList.add('phase-text');
    phaseTextElement.innerHTML = phaseText1 + " und " + phaseText2;
